...
use crate::resources::{EnemySpawnTimer, HighScores, Score, StarSpawnTimer};
    ...
    if let Ok(player_transform) = player_query.get_single() {
        if enemy_spawn_timer.timer.finished() {
            ...
            let random_x = min(
                ...
                window_width - half_enemy_size,
            );
            let random_y = min(
                max(
                    random::<f32>() * window_height,
                    player_transform.translation.y + 100.0,
                ),
                window_height - half_enemy_size,
            );
...
pub fn exit_game(
    ...
    if keyboard_input.just_pressed(KeyCode::Escape) {
        println!("User pressed Escape, exiting game.");
        app_exit_event_writer.send(AppExit);
    }
}

pub fn handle_game_over(mut game_over_event_reader: EventReader<GameOver>) {
    // multiple systems can send the same event type, so have to iterate
